...
async def build_report(period: str) -> str:
    ...
    # Abgeschlossene Deals (Won)
    won = api.get("opportunity", params={
        "status_type": "won",
        "date_won__gte": start,
        "date_won__lte": end,
        "_fields": "id,value,lead_name,status_label,date_won"
    })
    won_deals = won.get("data", [])
    won_value = sum(d.get("value", 0) or 0 for d in won_deals) / 100
    ...
    lost = api.get("opportunity", params={
        ...
        "date_lost__lte": end,
        "_fields": "id,value,lead_name,status_label"
    })
    lost_deals = lost.get("data", [])

    # Neue Leads
    new_leads = api.get("lead", params={
        "date_created__gte": start,
        "date_created__lte": end,
        "_fields": "id,display_name,status_label"
    })
    leads_list = new_leads.get("data", [])

    lines = [
        f"📊 *Sales Report — {label}*",
        f"📅 {datetime.now().strftime('%d.%m.%Y %H:%M')}",
        "",
        f"✅ *Won Deals:* {len(won_deals)} ({format_currency(won_value)})",
        f"❌ *Lost Deals:* {len(lost_deals)}",
        f"👥 *Neue Leads:* {len(leads_list)}",
    ]

    if won_deals:
        lines.append("\n*Top Won Deals:*")
        for d in won_deals[:5]:
            name = d.get("lead_name", "Unbekannt")
